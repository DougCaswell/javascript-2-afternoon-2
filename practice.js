...
function removeItem (mGL, item) {
  if (!item || !mGL) {return [];}
  else if (mGL.indexOf(item) >= 0) {
    mGL.splice(mGL.indexOf(item), 1);
    return mGL;
  }
  else return mGL;
}
function addItem (mGL, item) {
  if (!item || !mGL) {return [];}
  else {
    mGL.push(item);
    return mGL;
...
let user3 = {
  ...
  email: 'whatis6times7@gmail.com',
  password: 'whatsThePoint',
  username: '42'
}
